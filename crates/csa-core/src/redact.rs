//! Shared redaction utilities for persisted and model-visible session text.
    ...
    Some(RedactionPatterns {
        ...
        token: Regex::new(
            r#"(?ix)
                ...
                |
                \b(?:[a-z0-9_]*(?:access_token|refresh_token|id_token|api_key|token))\b\s*[:=]\s*["']?[^"',\s}]+["']?
            "#,
        )
        .ok()?,
        secret_kv: Regex::new(
            r#"(?ix)
                (?:
                    \b(?:[a-z0-9_]*(?:password|passwd|pwd|secret|client_secret|api_key|access_token|refresh_token|id_token|token))\b
                    \s*[:=]\s*
                    (?:
                        "(?:\\.|[^"])*"
                        |
                    ...
                    |
                    (?:\\?")(?:[a-z0-9_]*(?:password|passwd|pwd|secret|client_secret|api_key|access_token|refresh_token|id_token|token))(?:\\?")
                    \s*:\s*
                    (?:\\?")(?:\\.|[^"\\])*(?:\\?")
                )
            "#,
...
/// Redact sensitive material from plain text.
///
/// Applies the same regex-based patterns as [`redact_event`] but operates on
/// arbitrary text rather than structured JSON. Used to sanitize context
/// summaries and persisted model-facing artifacts.
#[must_use]
...
mod tests {
    ...
    #[test]
    fn test_redact_text_content_masks_prefixed_env_secret_keys_and_credential_urls() {
        let text = "\
safe before
...
safe after";
        ...
        let out = redact_text_content(text);
        ...
        for secret in [
            ...
            assert!(
                !out.contains(secret),
                "secret remained in redacted text: {secret}"
            );
        }
        assert!(out.contains("safe before"));
        assert!(out.contains("safe after"));
        assert!(out.contains("[REDACTED]"));
    }
}
