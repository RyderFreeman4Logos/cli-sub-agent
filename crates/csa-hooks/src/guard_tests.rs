...
mod unix_tests {
    ...
    fn test_run_guards_timeout_kills_escaped_descendants() {
        ...
        let unique_secs = 200_000 + std::process::id();
        let marker = format!("sleep {unique_secs}");
        ...
        let guards = vec![PromptGuardEntry {
            ...
            command: format!("setsid sh -c 'exec sleep {unique_secs}' </dev/null & sleep 10"),
            timeout_secs: 1,
        }];
        ...
        // Check that no process with our unique sleep duration is still running.
