...
#[test]
fn test_validate_tier_model_spec_unknown_provider_warns_and_is_admitted() {
    let dir = tempdir().unwrap();
    ...
    config.save(dir.path()).unwrap();
    let config_path = dir.path().join(".csa").join("config.toml");
    validate_config_with_paths(None, &config_path)
        .expect("configured provider/model should be admitted with a warning");
}

#[test]
fn test_validate_tier_model_spec_unknown_model_warns_and_is_admitted() {
    let dir = tempdir().unwrap();
    ...
    let config_path = dir.path().join(".csa").join("config.toml");
    validate_config_with_paths(None, &config_path)
        .expect("configured model should be admitted with a warning");
}
