...
pub enum VersionCheckResult {
    ...
    /// Callers should warn and leave the lock unchanged until explicit migrate.
    VersionDrift {
...
/// - Version differs, no pending migrations → return `VersionDrift` without
...
pub fn check_version(
    ...
    if pending.is_empty() {
        // No migrations needed — startup should stay read-only and let
        // explicit `csa migrate` update the tracked version.
        let _ = binary_weave_version;
...
mod tests {
    ...
    #[test]
    fn test_check_version_reports_version_drift_without_mutating_lockfile() {
        let dir = TempDir::new().unwrap();
        let lock = WeaveLock::new("0.1.0", "0.1.0");
        lock.save(dir.path()).unwrap();
