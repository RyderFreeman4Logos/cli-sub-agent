...
pub(crate) fn count_prior_reviews_for_branch(
    project_root: &Path,
    branch: Option<&str>,
    ...
    match branch {
        Some(branch) => {
            try_max_review_iterations_for_branch(project_root, branch, current_session_id)
...
pub(crate) fn render_review_iteration_context(
    project_root: &Path,
    branch: &str,
    current_session_id: Option<&str>,
) -> Option<String> {
    let prior_count =
        count_prior_reviews_for_branch(project_root, Some(branch), current_session_id);
    if prior_count == 0 {
        return None;
    }
