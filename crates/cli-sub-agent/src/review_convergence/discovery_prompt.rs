...
pub(crate) fn build_clean_room_prompt(
    ...
    format!(
        "Perform a fresh clean-room review of only immutable artifact csa-session://{}/{} with required SHA-256 {} for exact epoch {} (base {}, HEAD {}, diff {}). Do not use prior transcripts, parent context, hidden memory, or prior reviewer prose. Return exact JSON and no prose. The host, not you, binds the artifact, model evidence, campaign, epoch, gate artifact, digest, schema, path, policy, and execution identity. Do not return any of those fields, commands, or instructions. Use exactly this schema: {{\"findings\":[{{\"semantic_identity\":{{\"violated_invariant\":\"...\",\"trigger_failure_mode\":\"...\",\"primary_component\":\"...\",\"bug_class\":\"...\"}},\"review_text\":\"bounded factual review text\"}}],\"questions\":[],\"unchecked_items\":[],\"review_text\":\"bounded factual summary\"}}. A complete clean review has empty findings, questions, and unchecked_items but still supplies review_text.",
        artifact.csa_session_id(),
        artifact.path(),
        artifact.digest(),
        epoch.id(),
