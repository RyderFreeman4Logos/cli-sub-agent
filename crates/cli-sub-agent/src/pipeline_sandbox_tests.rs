...
fn test_none_config_sets_lean_mode_for_heavyweight() {
    let result = resolve_sandbox_options(None, "claude-code", "test-session", StreamMode::BufferOnly, 120);

    let SandboxResolution::Ok(opts) = result else {
        panic!("Expected SandboxResolution::Ok");
    };

    assert!(opts.lean_mode, "Heavyweight tool should have lean_mode=true");
}
...
fn test_none_config_lightweight_skips_sandbox() {
    let result = resolve_sandbox_options(None, "codex", "test-session", StreamMode::BufferOnly, 120);

    let SandboxResolution::Ok(opts) = result else {
        panic!("Expected SandboxResolution::Ok");
    };

    assert!(!opts.lean_mode, "Lightweight tool should have lean_mode=false");
    assert!(
...
fn test_none_config_heavyweight_gets_sandbox() {
    let result =
        resolve_sandbox_options(None, "claude-code", "test-session", StreamMode::BufferOnly, 120);
    ...
    if matches!(capability, csa_resource::SandboxCapability::None) {
        ...
        let ctx = opts.sandbox.as_ref().expect("Expected SandboxContext for heavyweight tool");
