...
#[path = "session_cmds_result_fix_finding_tests.rs"]
mod fix_finding;
...
fn handle_session_artifacts_reconciles_orphaned_active_session() {
    ...
    let result = load_result(project, &session_id)
        .unwrap()
        .expect("session artifacts command should reconcile missing terminal result");
    assert_eq!(result.status, "failure");
...
#[cfg(unix)]
#[test]
fn handle_session_artifacts_prefers_daemon_completion_packet() {
    let tmp = tempdir().unwrap();
    let _env_lock = TEST_ENV_LOCK.blocking_lock();
    let state_home = tmp.path().join("xdg-state");
    std::fs::create_dir_all(&state_home).unwrap();
    ...
    let session = create_session(project, Some("artifacts-completion-packet"), None, None).unwrap();
    let session_id = session.meta_session_id;
    let session_dir = get_session_dir(project, &session_id).unwrap();
    backdate_tree(&session_dir, 120);
    std::fs::write(
    ...
    let result = load_result(project, &session_id)
        .unwrap()
        .expect("session artifacts should synthesize from daemon completion");
    assert_eq!(result.status, "failure");
    assert_eq!(result.exit_code, 17);
