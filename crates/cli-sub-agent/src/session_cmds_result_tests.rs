...
#[cfg(target_os = "linux")]
fn read_process_start_time_ticks(pid: u32) -> anyhow::Result<u64> {
    let stat_path = format!("/proc/{pid}/stat");
    let content = std::fs::read_to_string(&stat_path)?;
    let close_paren = content
        .rfind(')')
        .ok_or_else(|| anyhow::anyhow!("missing stat comm terminator for pid {pid}"))?;
    let after_comm = &content[close_paren + 1..];
    let mut parts = after_comm.split_whitespace();
    parts
        .next()
        .ok_or_else(|| anyhow::anyhow!("missing process state for pid {pid}"))?;
    parts
        .next()
        .ok_or_else(|| anyhow::anyhow!("missing parent pid for pid {pid}"))?;
    parts
        .next()
        .ok_or_else(|| anyhow::anyhow!("missing process group for pid {pid}"))?;
    for _ in 0..16 {
        parts
            .next()
            .ok_or_else(|| anyhow::anyhow!("missing intermediate stat field for pid {pid}"))?;
    }
    parts
        .next()
...
#[cfg(target_os = "linux")]
fn daemon_pid_record(pid: u32) -> anyhow::Result<String> {
...
#[cfg(unix)]
#[test]
fn handle_session_result_preserves_available_session_on_daemon_completion() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let _env_lock = TEST_ENV_LOCK.blocking_lock();
    let state_home = tmp.path().join("xdg-state");
    ...
    let session = create_session(
        project,
        Some("result-available-completion-packet"),
        None,
        Some("codex"),
    )?;
    let session_id = session.meta_session_id;
    ...
    let mut persisted = csa_session::load_session(project, &session_id)?;
    persisted.phase = csa_session::SessionPhase::Available;
    csa_session::save_session(&persisted)?;
    ...
    csa_session::save_result(project, &session_id, &existing)?;
    let result_path = session_dir.join(csa_session::result::RESULT_FILE_NAME);
    let result_before = std::fs::read_to_string(&result_path)?;
    ...
    assert_eq!(persisted.phase, csa_session::SessionPhase::Available);
    assert_eq!(std::fs::read_to_string(&result_path)?, result_before);
...
#[cfg(target_os = "linux")]
#[test]
fn handle_session_result_ignores_completion_packet_while_daemon_alive() -> anyhow::Result<()> {
    ...
    let session = create_session(
        project,
        Some("result-live-completion-packet"),
        None,
        Some("codex"),
    )?;
    let session_id = session.meta_session_id;
    ...
    assert!(
        ...
        "session result must not synthesize result.toml while the daemon is still alive"
    );
    assert!(
        !session_dir
            .join(csa_session::result::RESULT_FILE_NAME)
            .exists(),
        "daemon completion packet must remain non-terminal while the daemon is still alive"
    );
