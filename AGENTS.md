...
026|background-task-hygiene|long-running tasks (>=4min) MUST background + poll; for non-CSA waits use `sleep "$(csa config get kv_cache.long_poll_seconds --default 240)" && check` + Read output file, while `csa session wait` MUST be called directly because it already applies `[kv_cache].long_poll_seconds` internally (legacy fallback 250 when `[kv_cache]` is absent); notifications are informational only, never decision signals; aligned with rule 032
...
031|sleep-interval-bands|Use either short polls (<=250s), the configured long-poll band (`kv_cache.long_poll_seconds`, default 240; Max-tier Opus commonly 3000), or >=18000s; arbitrary mid-band sleeps that are neither short nor the configured long poll are FORBIDDEN
032|background-long-running|>4min tasks (native subagent, `commit` with hook CI, `cargo build/test`) MUST run in background; for non-CSA waits the main agent polls via SINGLE `sleep "$(csa config get kv_cache.long_poll_seconds --default 240)" && <check>` Bash calls, while `csa session wait` MUST be called directly because it already uses `[kv_cache].long_poll_seconds` internally (legacy fallback 250 when `[kv_cache]` is absent); NEVER `for`/`while` loops, nested sleep, or chained polling in one call — each poll is ONE isolated Bash invocation that returns control to main agent so token generation between polls keeps KV cache warm; foreground blocking wastes KV + context rot
