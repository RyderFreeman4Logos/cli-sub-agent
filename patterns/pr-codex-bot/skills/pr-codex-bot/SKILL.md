...
10.5. **Rebase for clean history**: If branch has > 3 commits, create backup branch, soft reset to main, create logical commits by selectively staging, force push with lease, then trigger one final `@codex review` and wait for clean pass before merge. Skip if <= 3 commits or already logically grouped.
