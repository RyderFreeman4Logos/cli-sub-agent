...
Wait `cloud_bot_wait_seconds` (default: `kv_cache.frequent_poll_seconds`, 60s), then launch `patterns/pr-bot/scripts/pr-bot-wait.sh` with `cloud_bot_poll_max_seconds` (default: `600`) as its internal timeout and `cloud_bot_poll_interval_seconds` (default: `30`) as its shell-level poll interval.
...
- CSA daemon + session wait (configured by `cloud_bot_wait_seconds` + `cloud_bot_poll_max_seconds`, defaults: `kv_cache.frequent_poll_seconds = 60` and `cloud_bot_poll_max_seconds = 600`)
