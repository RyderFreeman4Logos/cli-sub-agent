...
CLOUD_BOT_POLL_MAX_SECONDS=$(csa config get pr_review.cloud_bot_poll_max_seconds --default "$(csa config get kv_cache.frequent_poll_seconds --default 60)")
...
Wait `cloud_bot_wait_seconds` (default: `kv_cache.frequent_poll_seconds`, 60s), then delegate `cloud_bot_poll_max_seconds` (same default) polling to CSA.
...
- CSA daemon + session wait (configurable via `cloud_bot_wait_seconds`, default: `kv_cache.frequent_poll_seconds = 60`)
...
- CSA daemon + session wait (configurable via `cloud_bot_wait_seconds`, default: `kv_cache.frequent_poll_seconds = 60`) enforces the hard timeout.
