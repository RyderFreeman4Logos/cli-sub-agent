...
if [ -z "$DEFAULT_BRANCH" ]; then
  # origin/HEAD not set locally — resolve from GitHub API as secondary source
  DEFAULT_BRANCH=$(gh repo view --json defaultBranchRef --jq '.defaultBranchRef.name' 2>/dev/null)
fi
if [ -z "$DEFAULT_BRANCH" ]; then
  echo "ERROR: cannot determine default branch (neither origin/HEAD nor 'gh repo view' succeeded)" >&2
  echo "Fix: run 'git remote set-head origin --auto' or set GH auth, then retry" >&2
  exit 1
fi
...
- CSA daemon + session wait (configured by `cloud_bot_wait_seconds` + `cloud_bot_poll_max_seconds`, defaults: `kv_cache.frequent_poll_seconds = 60` and `kv_cache.long_poll_seconds = 240`)
