...
```bash
set -euo pipefail
REVIEW_OUTPUT="$(csa review --sa-mode true --timeout 1800 --range "${DEFAULT_BRANCH}...HEAD" 2>&1)" || true
printf '%s\n' "${REVIEW_OUTPUT}"
...
set +e
csa review --sa-mode true --timeout 1800 --range "${DEFAULT_BRANCH}...HEAD" 2>&1 | tee "${REVIEW_OUTPUT_FILE}"
