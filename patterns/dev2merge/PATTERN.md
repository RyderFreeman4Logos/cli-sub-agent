...
```bash
set -euo pipefail
SID=$(csa review --sa-mode true --range "${DEFAULT_BRANCH}...HEAD") || true
REVIEW_OUTPUT="$(csa session wait --session "$SID" 2>&1)" || true
printf '%s\n' "${REVIEW_OUTPUT}"
...
csa session wait --session "$SID" 2>&1 | tee "${REVIEW_OUTPUT_FILE}"
