...
if printf '%s' "${RESOLVED_LANGUAGE}" | grep -qi 'chinese'; then
  ...
  CJK_COUNT_DRAFT=$(printf '%s\n' "${STEP_7_OUTPUT}" | rg -o '[\p{Han}\p{Hiragana}\p{Katakana}]' | wc -l | tr -d '[:space:]')
  [[ "${CJK_COUNT_DRAFT:-0}" -ge "${MIN_CJK}" ]] || { echo "STEP_7_OUTPUT language mismatch: expected CJK-script content (CJK chars >= ${MIN_CJK})" >&2; exit 1; }
fi
