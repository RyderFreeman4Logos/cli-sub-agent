...
"" \
"## Output Requirements" \
"Return EXACT section headers in ${LANGUAGE} (headers themselves remain English):" \
"DEBATE_EVIDENCE:" \
"- method: csa debate" \
"- rounds: 3" \
"- language: ${LANGUAGE}" \
...
"SUGGESTED_CHANGES:" \
"OVERALL_ASSESSMENT:" )"
printf '%s\n' "${DEBATE_PROMPT}" | csa debate --rounds 3 --format text
```
...
printf '%s\n' "${STEP_6_OUTPUT}" | grep -Eq 'verdict:[[:space:]]*(READY|REVISE)' || { echo "debate verdict missing" >&2; exit 1; }
