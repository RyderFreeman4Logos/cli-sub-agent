...
extract_spec() { perl -0CSDA -we '
my($f,$s)=(shift,<>);
my$h=$s;$h=~s/\A\s*//s;$h=~s/\n.*\z//s;exit 1 if $h=~/^(error:|fatal:|warning:|cargo:)/i;
sub ok{$_[0]=~/(^|\n)schema_version\s*=\s*1(\s*\n|$)/s&&$_[0]=~/(^|\n)plan_ulid\s*=/s&&$_[0]=~/(^|\n)\[\[criteria\]\](\s*\n|$)/s}
sub looks{$_[0]=~/(^|\n)\s*(schema_version|plan_ulid|summary)\s*=/s||$_[0]=~/(^|\n)\s*\[\[criteria\]\]/s}
sub head{my($b)=@_;$b=~s/\A\s*//s;$b=~s/\n.*\z//s;return $b=~/^(#|schema_version\s*=|plan_ulid\s*=|summary\s*=|\[\[criteria\]\])/}
sub emit{my($b)=@_;if(ok($b)||(looks($b)&&head($b))){print$b;exit 0}}
while($s=~/\Q$f\E[ \t]*(?:(?:toml|spec\.toml)(?:[ \t][^\n]*)?)?[ \t]*\n(.*?)\n\Q$f\E/sg){emit($1)}
while($s=~/<!--[ \t]*CSA:SECTION:[^>]*-->\s*(.*?)\s*<!--[ \t]*CSA:SECTION:[^>]*:END[ \t]*-->/sg){emit($1)}
emit($s);
exit 1' "${FENCE}" "$1"; }
h=$(sed -n 's/^[[:space:]]*//; /[^[:space:]]/{p;q;}' "${RAW_SPEC_ARTIFACT}") || { echo "read raw spec artifact failed" >&2; exit 1; }
if ! extract_spec "${RAW_SPEC_ARTIFACT}" > "${SPEC_ARTIFACT}"; then
  l="$h"
  lc=$(printf '%s' "$l" | tr '[:upper:]' '[:lower:]'); k="non-TOML"
  case "$lc" in error:*|fatal:*|warning:*|cargo:*) k="diag" ;; esac
  printf 'spec producer-contract error: expected TOML spec artifact; first content: %s\n' "$k" >&2
  if [ "$k" = "diag" ]; then printf 'underlying command failure: diag\nCommand stderr summary: %s\n' "$l" >&2; else printf 'parser/root-cause: no spec\n' >&2; fi
  printf 'Spec artifact path: %s\nRaw spec artifact path: %s\n' "${SPEC_ARTIFACT}" "${RAW_SPEC_ARTIFACT}" >&2
  exit 1
fi
...
v="${SAVE_DIR}/validate.stderr"
"${CSA_BIN}" todo persist --dry-run -t "${TODO_TS}" --todo-file "${TODO_ARTIFACT}" --spec-file "${SPEC_ARTIFACT}" "${EPIC_ARGS[@]}" "validate: ${FEATURE}" > /dev/null 2>"$v" || {
  rc=$?; printf 'spec producer-contract error: generated TODO/spec artifacts failed csa todo persist --dry-run (exit %s)\n' "$rc" >&2
  printf 'parser/root-cause:\n' >&2; [[ ! -s "$v" ]] || tail -80 "$v" >&2
  printf 'Spec artifact path: %s\nRaw spec artifact path: %s\n' "${SPEC_ARTIFACT}" "${RAW_SPEC_ARTIFACT}" >&2
  exit "$rc"
}
if printf '%s' "${RESOLVED_LANGUAGE}" | grep -qi 'chinese'; then
  ...
  HAN_COUNT=$(perl -CSDA -ne '$count += () = /\p{Han}/g; END { print $count + 0, "\n" }' "${TODO_ARTIFACT}")
  [[ "${HAN_COUNT:-0}" -ge "${MIN_HAN}" ]] || { echo "TODO Han chars < ${MIN_HAN}" >&2; exit 1; }
  ...
  CJK_COUNT=$(perl -CSDA -ne '$count += () = /[\p{Han}\p{Hiragana}\p{Katakana}]/g; END { print $count + 0, "\n" }' "${TODO_ARTIFACT}")
