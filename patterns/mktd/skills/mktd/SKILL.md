...
# Light mode (small changes — skips threat model + debate):
...
4. **Phase 2.5 -- THREAT MODEL** *(skipped in light mode)*: Review each new API surface for security concerns (sensitive data flows, hostile input, information exposure, safe defaults). Append findings as [Security] tagged items.
...
6. *(full mode only)* Threat model completed for all new API surfaces.
7. *(full mode only)* Adversarial debate completed via explicit `csa debate`.
